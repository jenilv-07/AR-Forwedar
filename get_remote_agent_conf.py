...
def handle_agent(agent_id):
    ...
    try:
        with MySocket(dest_socket) as s:
            ...
            return (agent_id, rec_msg_ok, rec_msg)
    except WazuhInternalError:
        print("WazuhInternalError ------------------------------------")
        return (agent_id, "error", "WazuhInternalError")
    except Exception as unhandled_exc:
        print(f"ERROR: {unhandled_exc}")
        return (agent_id, "error", str(unhandled_exc))

def process_agents(agent_list, timeout=5):
    ray.init(ignore_reinit_error=True)

    # Start the remote tasks
    futures = [handle_agent.remote(agent_id) for agent_id in agent_list]
